...
function element_add(element_type,position,content,addclassname=null,onclick=null,data=null){
        ...
        .then(data => {
            ...
            JSON.parse(data).forEach(function(element){
                if(element.display!==false){
                    element_add("section","main",`<h1>${element.title}</h1>${marked(element.content)}`);
